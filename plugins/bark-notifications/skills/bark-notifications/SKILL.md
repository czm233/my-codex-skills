...
- `bin/bark-stop-hook` runs for Stop events, ignores internal or ephemeral turns when their `session_id` differs from the visible `CODEX_THREAD_ID`, and deduplicates accepted events by `session_id + turn_id`. When `CODEX_THREAD_ID` is unavailable, it preserves the per-session behavior for compatibility. It reads only the current Thread's user-facing name via `thread/read` metadata and never reads transcript turns or message content.
- `bin/bark-task-complete` is the single status sender. It reads the Bark Device Key from the macOS Keychain, resolves the local machine label, and sends the machine label as the Bark title and the sanitized Session title as the notification body. If the Session title is unavailable, it sends the fixed fallback “本轮回复已结束”.
...
The plugin-bundled Stop Hook invokes these files through the runtime's plugin-root template variable: `${CLAUDE_PLUGIN_ROOT}` (also accepted as `${ZCODE_PLUGIN_ROOT}`) in ZCode, or `${PLUGIN_ROOT}` in Codex. `hooks/hooks.json` ships with the ZCode variable. Do not create a second Bark sender under `~/.codex/bin`, edit `~/.codex/hooks.json`, embed a `curl` request in a Hook definition, or make the Stop Hook continue the model. The Markdown Skill remains the source of Bark API and safety guidance, while the bundled commands provide the deterministic lifecycle integration.

For setting up this integration on another computer, read [references/codex-hook-setup.md](references/codex-hook-setup.md). It covers user-level plugin installation, the machine-label configuration contract, local Keychain storage, Hook trust review, dry-run verification, and troubleshooting without putting a Device Key in the repository or chat.
